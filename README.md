...
- On some Windows configurations, `mongorestore`/`pymongo` connections to MongoDB Atlas may fail with a TLS handshake error (`TLSV1_ALERT_INTERNAL_ERROR`). During development, this was traced to an unstable/poor local WiFi connection dropping or corrupting the TLS handshake with Atlas's servers — the same code and credentials worked reliably on a prior, more stable connection. The MongoDB connector logic itself was verified correct (confirmed identically at both the `mongorestore` CLI level and the raw `pymongo` driver level, ruling out a code-level bug). If you encounter this error: check your network stability first (try a different WiFi network or a mobile hotspot), then verify your system clock is accurate, and finally check for antivirus software performing HTTPS/TLS inspection.
